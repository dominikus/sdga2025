					...
					<defs>
						...
						>
							<line
								...
								y2="50"
								stroke-width="10"
								style="stroke: var(--color-grey-400);"
							/>
						</pattern>
		...
		{#if !$isScreenshotting}
			<div class="source">
				...
				<span class="download-share">
					<span class="download" class:borderless={!note || $isMobile}>
						{#if dataDownloadUrl && !hide_download}<a
								...
								><span aria-hidden="true" class:hidden={!dataDownloadUrl && !imageDownload}
									>{$_('data.download')}</span
								><!-- TODO: reenable <Fa icon={faDownload} />--></a
							>{/if}
					</span>
					{#if imageDownload}
						<span
							class="download"
							class:borderless={(!note && !dataDownloadUrl) ||
								(note && !dataDownloadUrl && $isMobile)}
						>
							<a
								class="download-icon"
								href={shareImg}
								target="_blank"
								rel="noreferrer"
								aria-label={$_('alt.download.image')}
								><span aria-hidden="true" class:hidden={!dataDownloadUrl && !imageDownload}
									>{$_('image.download')}</span
								><!-- TODO: reenable <Fa icon={faImage} /> --></a
							>
						</span>
					{/if}
					...
					<span
						class="share"
						class:borderless={!note && (!dataDownloadUrl || hide_download) && !imageDownload}
						><a
						...
						>
							<span aria-hidden="true">{$_('copy.link', { default: 'Copy Link' })}</span
							><!-- TODO: reenable <Fa
								icon={faLink}
							/>--></a
						></span
				...
				{#if mousePos || toastMode}
					<p class="toast" class:disappearing={toastMode} aria-hidden="true">
						{#if toastMode}{$_('link.copied', { default: 'Link copied to clipboard' })}
							<!-- TODO: reenable <Fa icon={faCheck} />-->{:else}{$_('click.copy', {
								default: 'Click to copy link'
							})}
							<!-- TODO: reenable <Fa icon={faLink} />-->{/if}
					</p>
				{/if}
			</div>
			{#if source}
...
<style type="scss">
	...
	:global(.rtl) .source .download,
	:global(.rtl) .source .share {
		margin-right: var(--space-s);
		padding-right: var(--space-s);
		margin-left: 0;
		padding-left: 0;
		border-right: 1px solid var(--color-theme-text-weaker);
		border-left: none;
	}
	.source .note {
		pointer-events: all;
	...
	:global(.rtl) .source :global(.svelte-fa),
	:global(.rtl) .toast :global(.svelte-fa) {
