...
<script>
    ...
    position: sticky;
		top: 0;
		transform: translate(0, ${typeof CSS !== 'undefined' && CSS.supports && CSS.supports('position', 'sticky') ? 0 : offset_top}px);
		z-index: ${inverted ? 3 : 1};
	`;

	$: widthStyle = fixed ? `width:${width}px;` : '';

	function findObservableScenes() {
		if (!count) {
			sections = foreground?.querySelectorAll(query);

			if (sections && sections.length === expected) {
				count = sections.length;

				update();
			} else {
				// wait a bit
				setTimeout(findObservableScenes, 1250);
			}
		}
	}

	onMount(() => {
		findObservableScenes();
